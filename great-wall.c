...
int main() {
    ...
    // Definimos a quantidade de lugares passáveis (inicialmente dá pra passar por todos os lugares)
    for(int i = 0; i < n; i++) {
        for(int j = 0; i < m; j++) {
            matriz[i][j].terreno = '.';
    ...
    no **adjResidual = malloc(n * m * sizeof(no*));
    no **next = malloc(n * m * sizeof(no*));
    no **listaGargalos = malloc(n * m * sizeof(no*));
    int* cor = malloc(n * m * sizeof(int));
    int* alcancavel = malloc(n * m * sizeof(int));
    ...
    *qtdCorte = 0;
    ...
    scanf("%d %d", &c1, &c2);
    matriz[c1][c2].terreno = "A";
    scanf("%d %d", &c1, &c2);
    matriz[c1][c2].terreno = "B";
